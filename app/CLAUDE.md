...
Warm Utah-canyon ramp. Defined in `tailwind.config.ts`. Use Tailwind utilities
— never inline hex except in `globals.css` tokens.

| Token       | Use                                                       |
|-------------|-----------------------------------------------------------|
| `paper`     | `#fbfaf7` — full-page background. Always.                 |
| `ink`       | `#1f1f1f` — primary text + dark CTAs.                     |
| `orange-500`| `#ff7227` — single brand accent. Primary buttons + key CTAs only. |
| `orange-600`| Hover state for primary CTAs.                             |
| `orange-50/100/200` | Soft pill backgrounds, subtle hover washes.       |
| `warmgray-50→900` | Neutrals. `100` for borders, `400` for muted labels, `600` for body, `700` for body-strong, `ink` for headlines. |
| `sand-*`    | Reserved for ecosystem-graph / Utah-signal moments.       |
| `emerald-*` | Live-mode badge dot, mutual-match success states. Sparingly. |
| `red-*`     | Form errors only. Never decorative.                       |

**Rules**
- One primary color (orange) per page. Don't add a second accent.
...
Loaded once in `globals.css`:

- **Source Serif 4** — `font-serif`. Headlines and section titles only.
  Tracking `-0.02em` on H1, default elsewhere. Weights 600/700.
- **Inter** — `font-sans` (body default). Weights 400/500/600/700.
...
Color the eyebrow `text-orange-500` for primary sections, `text-warmgray-400`
for secondary, `text-emerald-700` for live-mode contexts.
...
- **Primary** — orange capsule:
  `inline-flex h-10 items-center justify-center rounded-full bg-orange-500 px-5 text-sm font-semibold text-white shadow-[0_8px_24px_-8px_rgba(255,114,39,0.55)] transition hover:bg-orange-600`.
...
Initials fallback over orange-100, with photo passthrough. Sizes `sm | md | lg`
...
- Hover-lift CTAs use the orange shadow grow:
  `hover:shadow-[0_10px_30px_-8px_rgba(255,114,39,0.7)]`.
